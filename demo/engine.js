...
(function () {
  ...
  function render() {
    ...
    if (s.type === 'divider') {
      ...
      const typeName = typeLabels[s.type] || '';
      slideEl.className = 'slide' + (s.type ? ` slide-${s.type}` : '');
      
      // 左：概念欄
      const info = document.createElement('div');
      info.className = 'slide-info';
      let html = `<div class="badge type-${s.type || 'concept'}">第 ${s.ch} 章 · ${s.sec} ${s.secName || ''}${typeName ? `<span class="type-pill">${typeName}</span>` : ''}</div>
        <h2 class="slide-title">${s.title}</h2>`;
      
      if (s.inquiry) {
        html += `<div class="inquiry-banner"><span class="ib-tag">💡 探究任務</span><div>${s.inquiry}</div></div>`;
      }
      if (s.formula) {
      ...
      if (s.example) {
        html += `<div class="example">
          <div class="ex-head">${s.type === 'practice' ? '隨堂練習' : '典例精析'}<button class="ex-zoom" title="放大成整頁，方便用畫筆講解">🔍 放大</button></div>
          <div class="ex-q">${s.example.q}</div>`;
        
        // 階梯式提示按鈕列
        let btnHtml = '<div class="ex-btn-group">';
        if (s.example.thinking) btnHtml += `<button class="ex-thinking-btn">💡 審題思路</button>`;
        if (s.example.hints && s.example.hints.length) btnHtml += `<button class="ex-hint-btn">🔑 啟發提示</button>`;
        btnHtml += `<button class="ex-toggle">顯示解答</button>`;
        if (s.example.variant) btnHtml += `<button class="ex-variant-btn">🎯 變式拓展</button>`;
        btnHtml += '</div>';
        html += btnHtml;

        if (s.example.thinking) {
          html += `<div class="ex-thinking-box"><b>💡 審題引導：</b>${s.example.thinking}</div>`;
        }
        if (s.example.hints && s.example.hints.length) {
          html += `<div class="ex-hints-box"><b>🔑 解題提示：</b><ol style="margin:4px 0 0;padding-left:18px">${s.example.hints.map(h => `<li>${h}</li>`).join('')}</ol></div>`;
        }
        html += `<div class="ex-sol">
            ${s.example.steps ? `<ol>${s.example.steps.map(t => `<li>${t}</li>`).join('')}</ol>` : ''}
            ${s.example.ans ? `<div class="ex-ans">答：${s.example.ans}</div>` : ''}
          </div>`;
        if (s.example.variant) {
          html += `<div class="ex-variant-box">
            <div class="ex-variant-title">🎯 即堂變式練習</div>
            <div class="ex-variant-q">${s.example.variant.q}</div>
            ${s.example.variant.ans ? `<div style="margin-top:6px;font-size:13.5px;color:#86198f"><b>參考答案：</b>${s.example.variant.ans}</div>` : ''}
          </div>`;
        }
        html += `</div>`;
      }
      ...
      // 階梯式提示互動綁定
      const thinkBtn = info.querySelector('.ex-thinking-btn');
      if (thinkBtn) {
        const tBox = info.querySelector('.ex-thinking-box');
        thinkBtn.onclick = () => {
          tBox.classList.toggle('show');
          thinkBtn.classList.toggle('active');
          typesetAndFit(tBox);
        };
      }
      const hintBtn = info.querySelector('.ex-hint-btn');
      if (hintBtn) {
        const hBox = info.querySelector('.ex-hints-box');
        hintBtn.onclick = () => {
      ...
      const varBtn = info.querySelector('.ex-variant-btn');
      if (varBtn) {
        const vBox = info.querySelector('.ex-variant-box');
        varBtn.onclick = () => {
          vBox.classList.toggle('show');
          varBtn.classList.toggle('active');
          typesetAndFit(vBox);
        };
      }
